...
def extractbackup(mflcode, backup):
    ...
    if(sqlfile != False):
        ...
        if(response['status']):
            db_status = True
            print('Started:', filename, 'was imported in', time.strftime(
                '%H:%M:%S', time.gmtime(time.time()-start)))
        else:
            print('Error:', filename, ' failed to be imported in',
...
def importsql(cfg, db_name, sqlfile):
    ...
    try:
        ...
        import_command = "mysql -h "+cfg["database"]["hostname"]+" -P "+cfg["database"]["port"] + \
            " -u "+cfg["database"]["username"]+" -p" + \
            cfg["database"]["password"]+" "+db_name+" < "+sqlfile
...
def logbackup(cfg, filename, mflcode, adt_version):
    try:
        ...
        # Get facility_id
        cursor.execute(
            "SELECT id FROM tbl_facility WHERE mflcode = '"+mflcode+"'")
        row = cursor.fetchone()
        # Log backupfile
        if row is not None:
            facility_id = row[0]
            cursor.execute("INSERT INTO tbl_backup(filename, foldername, adt_version, facility_id) VALUES (%s, %s, %s, %s)",
                           (filename, mflcode, adt_version, facility_id))
...
def extractdata(cfg, source_db, mflcode):
    ...
    filestream = getsql(fixfile).format(
        db_name=source_db).strip().replace('\r\n', '')
    ...
    for source_table in source_tables:
        ...
        sql_counter = 0
        sqlfile = cfg['main']['queries_dir'] + \
            'select_'+source_table+'_table.sql'
        destination_table = str(cfg['destination'][source_table])
        # Get table_size
        sql_count = "SELECT COUNT(*) total FROM " + \
            source_table + " WHERE active = '1'"
        try:
            with warnings.catch_warnings():
                source_cursor = source_cnx.cursor()
            ...
            if(err.args[0] == 1146):
                print("Table does not exist, moving next ...")
            else:
                print("Table exists")
        # Use batch to get data
        print('Info: Migration of table', source_table,
              'containing rows', table_size, 'has started!')
        while sql_counter < table_size:
            try:
                with warnings.catch_warnings():
                    warnings.filterwarnings("ignore")
                    source_cursor = source_cnx.cursor()
                    query = getsql(sqlfile).format(
                        mflcode, batch_size, sql_counter)
                ...
                print('Info: Migration of table', source_table,
                      'imported rows', sql_counter, 'currently in progress!')
            except Exception as e:
                logfile = cfg['main']['logs_dir']+'failed_'+source_table+'.log'
                message = "Database: " + source_db + " Error: " + str(e)
                writelog(logfile, message)
        extract_status = True
        print('Success: Migration of table', source_table, 'completed in',
              time.strftime('%H:%M:%S', time.gmtime(time.time()-inner_start)))
    # Close source connections
    source_cursor.close()
    source_cnx.close()
    # Display message
    print('Finished: Database', source_db, 'data was extracted in',
          time.strftime('%H:%M:%S', time.gmtime(time.time()-start)))
...
def savebatch(cfg, destination_table, source_data):
    ...
    try:
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore")
            target_cursor.executemany(getsql(
                cfg['main']['queries_dir']+'insert_'+destination_table+'_table.sql'), source_data)
...
def runproc(cfg, proc_name):
    ...
    try:
        ...
        target_cursor.close()
        target_cnx.close()
        print('Success: Procedure', proc_name, 'completed in',
              time.strftime('%H:%M:%S', time.gmtime(time.time()-start)))
    except Exception as e:
...
if __name__ == '__main__':
    ...
    for mflcode, backup in backups.items():
        for db_name, db_status in extractbackup(mflcode, backup).items():
            if db_status:
                # Extract data
            ...
            # Drop source database
                ...
                print(mflcode+" "+"Is an invalid file")
